...
");
    ...
    [Test]
    public void can_use_custom_mappings_between_database_and_dotnet()
    {
        try
        {
            Demapio.Demapio.SetTypeMapping(WeirdType.FromText, WeirdType.ToText);

            var conn = new NpgsqlConnection(ConnStr);

            // Create a test table
            conn.QueryValue(@"
CREATE TABLE IF NOT EXISTS WeirdTypeTable (
    id INT,
    typeValue TEXT
);
TRUNCATE WeirdTypeTable;
");

            conn.RepeatCommand("INSERT INTO WeirdTypeTable (id, typeValue) VALUES (:id, :typeValue);",
                new { id = 1, typeValue = new WeirdType("Hello")},
                new { id = 2, typeValue = new WeirdType("World")}
            );


            var result = conn.SelectType<WeirdContainer>("SELECT * FROM WeirdTypeTable;", new { }).ToList();
